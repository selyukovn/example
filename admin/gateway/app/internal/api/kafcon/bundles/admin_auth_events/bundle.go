...
import (
	...
	"example/admin/gateway/internal/api/kafcon/bundles/admin_auth_events/handlers"
...
) kernel.HandlerInterface {
	return kernel.FnHandler(func(ctx context.Context, kMsg *kafka.Message) error {
		_o_ := TopicName
		_m_ := "NewHandlerDefault"

		service := sDecoratorBoundary{
			sDefault{
				sessionClosedV1: handlers.NewSessionClosedV1(sAuthCacher),
			},
		}
...
func NewHandlerDecoratorDlq(
	origin kernel.HandlerInterface,
	...
	fnGetGroupId := func(ctx context.Context, kMsg *kafka.Message) (string, error) {
		// Внимание!
		//
...
// Decorator DLQ: Kafapi Service to Parse Metadata
// ---------------------------------------------------------------------------------------------------------------------
