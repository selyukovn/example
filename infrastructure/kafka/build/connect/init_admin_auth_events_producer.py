...
# У Debezium, как оказалось, есть специальный трансформатор под outbox-паттерн,
# но узнал я об этом уже после того, как настроил этот коннектор.
# https://debezium.io/documentation/reference/3.5/transformations/outbox-event-router.html
# Переделывать не стал, т.к. outbox-трансформатор накладывает ограничения на схему хранения и другие трансформаторы.
# Потому этот коннектор остается, каким он и был, а на outbox'е, возможно, будет другой -- для разнообразия.

# --

allBrokersString = os.environ["CONNECT_BOOTSTRAP_SERVERS"]
brokersCount = max(1, len(allBrokersString.split(",")))
# Количеству партиций желательно быть кратным количеству брокеров для равномерного распределения.
# Количеству партиций желательно быть > 1 для предоставления потребителям возможности распараллеливать обработку.
# Количеству партиций желательно быть не слишком большим без необходимости для экономии ресурсов.
# Не ожидается, что admin-auth-сервис будет генерировать много событий, поэтому достаточно небольшого кол-ва партиций.
publicTopicPartitionsCount = min(3, brokersCount)

# Внимание!
# Debezium по умолчанию предоставляет гарантию `at-least-once` (дубликаты возможны при восстановлении после сбоев).
# Некоторые коннекторы поддерживают гарантию `exactly-once`, но нет ни необходимости, ни желания ее настраивать.
# https://debezium.io/documentation/reference/3.5/connectors/mysql.html#mysql-when-things-go-wrong
# https://debezium.io/documentation/reference/3.5/configuration/eos.html

# --

connectorName = "mysql-admin-auth-event-producer"
dbTables = "auth.event"
topicRedirectFromRegexp = "mysql-admin-auth-event-producer.auth.event"
topicRedirectTo = "admin-auth-events"
partitionKeyingFields = "change.outbox_group_id"
...
req = urllib.request.Request(
    ...
    data=json.dumps({
        ...
        "config": {
            ...
            # В топике `connectorName` будет DDL -- он не нужен для продюсирования событий.
            # Но для топика с данными это снижает надежность и лишает возможности параллельно обрабатывать события,
            # т.е. в рамках одного сервиса-приемника может быть запущен только один консьюмер на группу
            ...
            "topic.creation."+topicRedirectTo+".partitions": publicTopicPartitionsCount,
            ...
            "transforms.Partitioning.partition.topic.num": publicTopicPartitionsCount,
